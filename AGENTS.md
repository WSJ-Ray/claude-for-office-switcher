...
DeepSeek Office Gateway — a Python FastAPI backend + React/Vite admin frontend that proxies the Codex Office add-in's Anthropic Messages API to upstream providers (DeepSeek, Moonshot, OpenAI, etc.). The gateway supports multi-provider routing, automatic failover by priority, streaming SSE forwarding, format translation (Anthropic ↔ OpenAI Chat Completions), and a SQLite-backed admin panel.
...
- **client_model mapping rule**: `client_model` must contain one of `sonnet`, `opus`, or `haiku` (enforced in admin routes). This is because the Codex Office add-in only recognizes model IDs containing these tokens.
